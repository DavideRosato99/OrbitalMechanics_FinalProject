...
mu_M = astroConstants(11);  % [km^3/s^2] Mercury's planetary constants
mu_V = astroConstants(12);  % [km^3/s^2] Venus's planetary constants
mu_J = astroConstants(15);  % [km^3/s^2] Jupiter's planetary constants
...
mean_radius_M = astroConstants(21);
mean_radius_V = astroConstants(22);
mean_radius_J = astroConstants(25);
Ratio_rs = mean_radius_S / mean_radius_M; % ratio between Sun's radius and Planet_1's radius
Ratio_r = mean_radius_M / mean_radius_V; % ratio between Planet_1's radius and Planet_2's radius
% Ratio_r = mean_radius_V / mean_radius_J; % ratio between Planet_1's radius and Planet_2's radius

t_start_1 = [ 2027, 6, 1, 0, 0, 0];
t_end_1   = [ 2042, 6, 1, 0, 0, 0];
t_start_2 = [ 2027, 6, 1, 0, 0, 0];
t_end_2   = [ 2042, 6, 1, 0, 0, 0];
...
for ii = 1 : length(T_1)
    ...
    [ r_1(ii,:),v_1(ii,:) ] = kep2car( kep_1(ii,1), kep_1(ii,2), kep_1(ii,3), kep_1(ii,4), kep_1(ii,5), kep_1(ii,6), mu_S );
end
for ii = 1 : length(T_2)
    [kep_2(ii,:)] = uplanet( T_2(ii), p2 );
    [ r_2(ii,:),v_2(ii,:) ] = kep2car( kep_2(ii,1), kep_2(ii,2), kep_2(ii,3), kep_2(ii,4), kep_2(ii,5), kep_2(ii,6), mu_S );
end
...
[kep_1_arr(:),~] = uplanet( T_2(j), p1 );
    [ r_1_arr(:),v_1_arr(:) ] = par2car( kep_1_arr(1), kep_1_arr(2), kep_1_arr(3), kep_1_arr(4), kep_1_arr(5), kep_1_arr(6), mu_S );
%evaluate where is the Planet 2 at departure time
[kep_2_arr(:),~] = uplanet( T_1(i), p2 );
    [ r_2_dep(:),v_2_dep(:) ] = par2car( kep_2_arr(1), kep_2_arr(2), kep_2_arr(3), kep_2_arr(4), kep_2_arr(5), kep_2_arr(6), mu_S );
%% Plot Orbit Opt
Orbit_switch = 0;
if(Orbit_switch)
...
%% SOI
r_SOI = norm(r_V*AU)*(mu_V/mu_S)^(2/5); % [km] radius of the Sphere of Influence
