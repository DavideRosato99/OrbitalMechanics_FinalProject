function [ DV, T_1, T_2, DV1, DV2, r_1, r_2, v_1, v_2, v_1_l, v_2_l ] = deltaV2( p1, p2, T_1, t_start_j_2, t_end_j_2, s_size )
% deltaV2 evaluate the Labert Problem
%
% PROTOTYPE:
%   [ DV, T_1, T_2, DV1, DV2, r_1, r_2, v_1, v_2, v_1_l, v_2_l ] = deltaV2( p1, p2, T_1, t_start_j_2, t_end_j_2, s_size )
%
% INPUT:
%	p1[1]          ID planet 1
%	p2[1]          ID planet 2
...
%   s_size[1]       size of the vector of dates on the arrival orbit
%
% OUTPUT:
...
% T_1 = linspace( t_start_j_1, t_end_j_1, s_size); % Date range on the departure orbit
T_2 = linspace( t_start_j_2, t_end_j_2, s_size)'; % Date range on the arrival orbit
%% Evaluate orbital parameters
for ii = 1 : length(T_1)
    [kep_1(ii,:)] = uplanet( T_1(ii), p1 );
    [ r_1(ii,:),v_1(ii,:) ] = kep2car( kep_1(ii,1), kep_1(ii,2), kep_1(ii,3), kep_1(ii,4), kep_1(ii,5), kep_1(ii,6), muS );
end
for ii = 1 : length(T_2)
    [kep_2(ii,:)] = uplanet( T_2(ii), p2 );
    [ r_2(ii,:),v_2(ii,:) ] = kep2car( kep_2(ii,1), kep_2(ii,2), kep_2(ii,3), kep_2(ii,4), kep_2(ii,5), kep_2(ii,6), muS );
end
for ii = 1 : length(T_1)
    for jj = 1 : length(T_2)
