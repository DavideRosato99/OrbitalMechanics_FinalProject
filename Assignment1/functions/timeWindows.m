function data = timeWindows(data, settings)
% timewindows - the function decreases the total time windows to find the 
% optimal time windows in which the final result is more likely to be found
%
% INPUTS:
...
%   data       struct  [1x1]   general data struct                  [-]
% 
% CALLED FUNCTIONS: date2mjd2000, uplanet, par2car, lambertMR
%
% CONTRIBUTORS:
...
fprintf('Delta V calculations:                       ');
if settings.timeWindows.parallel
...
%% FIND OPTIMAL FLY-BY DATE INTERVAL
daysSyn1 = synPeriod1/(24*3600);
daysSyn2 = synPeriod2/(24*3600);

minDV     = nan(floor(TOTdays*24*3600/synPeriod2)+1, 1);
minDVdays = nan(floor(TOTdays*24*3600/synPeriod2)+1, 1);
...
f = @(DV) (maxMinDV - DV)/(maxMinDV - minMinDV);
figure
plot(minDVdays, minDV, 'o-', 'LineWidth', 1); grid on; hold on;
xticks(vec2); xticklabels(vec1); xtickangle(-45)
xline(minDVdays(ind), 'r-', 'Best');
ylabel('$\Delta$V [$\frac{km}{s}$]'); xlabel('Earth Departure Date');
title('$\Delta$V Second Impulse Second Leg')

dateOptFlyBy = (sum(minDVdays(not(isnan(minDV))).*f(minDV(not(isnan(minDV))))))/(sum(f(minDV(not(isnan(minDV))))));

figure
plot(minDVdays, f(minDV), 'o-', 'LineWidth', 1); hold on; grid on
xticks(vec2); xticklabels(vec1); xtickangle(-45)
xline(dateOptFlyBy, 'b-', 'Weighted'); xline(minDVdays(ind), 'r-', 'Best');
ylabel('w($\Delta$V) [-]'); xlabel('Earth Departure Date');
title('$\Delta$V Second Impulse Second Leg - Transformed')
...
maxMinDV = max(minDV2);
[minMinDV, ind] = min(minDV2);

vec1 = '';
vec2 = linspace(0, TOTdays, 30);
...
f = @(DV) (maxMinDV - DV).^0.1/(maxMinDV - minMinDV)^0.1;

figure
plot(minDVdays2, minDV2, 'o-', 'LineWidth', 1); grid on; hold on;
xticks(vec2); xticklabels(vec1); xtickangle(-45)
xline(minDVdays2(ind), 'r-', 'Best');
ylabel('$\Delta$V [$\frac{km}{s}$]'); xlabel('Mars Departure Date');
title('$\Delta$V First Impulse First Leg')

dateOptDep = (sum(minDVdays2(not(isnan(minDV2))).*f(minDV2(not(isnan(minDV2))))))/(sum(f(minDV2(not(isnan(minDV2))))));

figure
plot(minDVdays2, f(minDV2), 'o-', 'LineWidth', 1); hold on; grid on
xticks(vec2); xticklabels(vec1); xtickangle(-45)
xline(dateOptDep, 'b-', 'Weighted'); xline(minDVdays2(ind), 'r-', 'Best');
ylabel('w($\Delta$V) [-]'); xlabel('Mars Departure Date');
title('$\Delta$V First Impulse First Leg - Transformed')
...
maxMinDV = max(minDV3);
[minMinDV, ind] = min(minDV3);

f = @(DV) (maxMinDV - DV)/(maxMinDV - minMinDV);

figure
plot(minDVdays3, minDV3, 'o-', 'LineWidth', 1); grid on; hold on;
xticks(vec2); xticklabels(vec1); xtickangle(-45)
xline(minDVdays3(ind), 'r-', 'Best');
ylabel('$\Delta$V [$\frac{km}{s}$]'); xlabel('Venus Arrival Date');
title('$\Delta$V Second Impulse Second Leg')
...
figure
plot(minDVdays3, f(minDV3), 'o-', 'LineWidth', 1); hold on; grid on
xticks(vec2); xticklabels(vec1); xtickangle(-45)
xline(dateOptArr, 'b-', 'Weighted'); xline(minDVdays3(ind), 'r-', 'Best');
ylabel('w($\Delta$V) [-]'); xlabel('Venus Arrival Date');
title('$\Delta$V Second Impulse Second Leg - Transformed')
...
data.timeWindows.maxTOF1days = dateOptFlyBy - dateOptDep;
data.timeWindows.maxTOF2days = dateOptArr - dateOptFlyBy;
%%
vec1 = '';
vec2 = linspace(0, TOTdays, 15);
...
if settings.timeWindows.plot
    ...
    contour(A, B, data.timeWindows.DVfirstLeg', 5000)
    xticks(vec2); xticklabels(vec1); xtickangle(-45)
    yticks(vec4); yticklabels(vec3),  grid on;
    xlabel('Mars Departure Date'); ylabel('Earth Arrival Date')
    title('$\Delta$V First Impulse First Leg Porkchop')
    
    figure
    [A, B] = meshgrid(data.timeWindows.FlyByDays, data.timeWindows.arrDays);
    contour(A, B, data.timeWindows.DVsecondLeg2', 1000)
    xticks(vec2); xticklabels(vec1); xtickangle(-45)
    yticks(vec4); yticklabels(vec3); grid on;
    xlabel('Earth Departure Date'); ylabel('Venus Arrival Date')
    title('$\Delta$V Second Impulse Second Leg Porkchop')
